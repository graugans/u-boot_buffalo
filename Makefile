...
#BUFFALO_BIN_VER
ifndef BUFFALO_BIN_VER
	BUFFALO_BIN_VER := 1.05
	export BUFFALO_BIN_VER
endif
...
ifdef BUFFALO_BIN_VER
ifdef IMAGEPATH
		if grep RD_BB08009 include/config.h > /dev/null ; then \
			cp u-boot.bin $(IMAGEPATH)/u-boot_$(BOARD)_$(BUFFALO_BIN_VER)_JP.bin;	\
		elif grep RD_BB08011 include/config.h > /dev/null ; then \
			cp u-boot.bin $(IMAGEPATH)/u-boot_$(BOARD)_$(BUFFALO_BIN_VER)_EU.bin;	\
		else	\
			cp u-boot.bin $(IMAGEPATH)/u-boot_$(BOARD)_$(BUFFALO_BIN_VER).bin;	\
		fi
endif
endif
...
ifneq ($(CONFIG_BUFFALO),y)
		...
		@echo -n "#define U_BOOT_VERSION \"U-Boot " > $(VERSION_FILE); \
		echo -n "$(U_BOOT_VERSION) for BUFFALO AIRSTATION" >> $(VERSION_FILE); \
		echo -n $(shell $(CONFIG_SHELL) $(TOPDIR)/tools/setlocalversion \
			 $(TOPDIR)) >> $(VERSION_FILE); \
		echo "\"" >> $(VERSION_FILE)
endif
...
WHR-HP-G300N_config		: 	unconfig
	@ >include/config.h
	@echo "#define CONFIG_AR7240 1" >>include/config.h
#	@echo "#define CONFIG_BUFFALO" >>include/config.h
#	@echo "#define CONFIG_BUFFALO_DEBUG" >>include/config.h
	@echo "#define CONFIG_BUFFALO_NO_ARP" >>include/config.h
	@echo "#define CONFIG_WHR_HP_G300N 1" >>include/config.h
	@echo "#define PRODUCT_NAME WHR-HP-G300N" >> include/config.h
	@echo "#define BUFFALO_BIN_VER $(BUFFALO_BIN_VER)" >> include/config.h
	@echo "#define BUFFALO_MELCO_ID \"RD_BB09017\"" >> include/config.h
	@echo "#define BUFFALO_HW_REV \"0\"" >> include/config.h
	@./mkconfig -a WHR-HP-G300N mips mips WHR-HP-G300N ar7240 ar7240
